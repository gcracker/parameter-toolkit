  ...
  )
{
  ...
  if (stringFirst == NULL) {
    switch (operationFlag) {
      ...
      case Pdtutil_MgrOpStats_c:
        fprintf(stderr,"not implemented array \n");
	/*
  ...
  )
{
  ...
  while (fgets(buf,100,fp)!=NULL) {
    ...
    if (name[0]=='$') {
      ...
      printf("loading constrain from %s\n",name1);
      ...
      if (idOrName) {
        ...
        while(name[0]==' ') name++;
        sscanf(name,"%s",name1);
        n = Ddi_VarIndex(Ddi_VarFromName(dd,name1));
        if (op2) {
          sscanf(name,"%*s %s",name2);
          n2 = Ddi_VarIndex(Ddi_VarFromName(dd,name2));
        }
      ...
      if (n <= 0)
        break;
      V = Ddi_BddMakeLiteral(Ddi_IthVar (dd,n),1);
      if (op2) {
        V2 = Ddi_BddMakeLiteral(Ddi_IthVar (dd,n2),1);
        switch (op2) {
          case '&':          
            Ddi_BddAndAcc(V,V2);
            break;
          ...
          case '=':          
            ...
            break;
          default:
            printf ("unknown operator in cube file %c\n",op2);
        }
        Ddi_Free(V2);
      }
    }  
...
)
{
  ...
  while (fscanf(fp,"%s",buf)>0) {
    if (idOrName) 
      ...
      n = Ddi_VarIndex(Ddi_VarFromName(dd,buf));
    vars = Ddi_VarsetEvalFree(
             Ddi_VarsetAdd (vars, Ddi_IthVar (dd,n)),
           vars);
  }
  return vars;
}
  ...
  Ddi_Vararray_t *array )
{
  ...
  for (i=0;i<Ddi_VararrayNum(array);i++) 
    printf("%d ",Ddi_VarIndex(Ddi_VararrayRead(array,i)));
  printf("\n");
  fflush(stdout);
}
