...
/**Function********************************************************************
  ...
  Description [outputs a 1 byte node code using the following format:
    ...
    for ZDDs the code is as follows:
     <pre>
     V           : 2 bits;     (variable code)
     T           : 3 bits;     (Then code)
     E           : 3 bits;     (Else code)
...
/**Function********************************************************************
  ...
  Synopsis    [Writes 1 byte node code for ZDDs]
...
DddmpZddWriteCode (
  FILE *fp                   /* IN: file where to write the code */,
  struct binary_zdd_code code /* IN: the code to be written */
  )
{
  unsigned char c;
  int retValue;

  c = ((code.V<<6)|(code.T<<3)|(code.E));
...
/**Function********************************************************************
  ...
  Synopsis    [Reads a 1 byte node code for ZDDs]

  Description [Reads a 1 byte node code. See DddmpZddWriteCode()
    for code description.]

  SideEffects [None]

  SeeAlso     [DddmpZddWriteCode()]
...
DddmpZddReadCode (
  ...
  struct binary_zdd_code *pcode /* OUT: the read code */
  )
{
  unsigned char c;
