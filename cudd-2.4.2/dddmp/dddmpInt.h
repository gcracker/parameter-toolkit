...
#define DDDMP_TERMINAL_ONE        0
#define DDDMP_TERMINAL_ZERO       0
...
/**Struct**********************************************************************
 Synopsis    [used in binary mode to store code info of a zdd node]
 Description [V , T , E store the mode used to represent variable, Then 
              and Else indexes. An index is either an absolute
	      ( DDDMP_ABSOLUTE_ID ),
              a relative numbers ( DDDMP_RELATIVE_ID , DDDMP_RELATIVE_1 ) or 
 ...
 SideEffect  [none]
 SeeAlso     [DddmpZddWriteCode DddmpZddReadCode] 
******************************************************************************/

struct binary_zdd_code {
  unsigned  V      : 2;
  unsigned  T      : 3;
  unsigned  E      : 3;
};
...
extern int DddmpNumberZddNodes(DdManager *ddMgr, DdNode **f, int n);
extern void DddmpUnnumberZddNodes(DdManager *ddMgr, DdNode **f, int n);
extern void DddmpWriteNodeIndexZdd(DdNode *f, int id);
extern int DddmpReadNodeIndexZdd(DdNode *f);
extern int DddmpVisitedZdd(DdNode *f);
extern void DddmpSetVisitedZdd(DdNode *f);
extern void DddmpClearVisitedZdd(DdNode *f);
...
extern int DddmpCuddBddArrayStore(Dddmp_DecompType ddType, DdManager *ddMgr, char *ddname, int nRoots, DdNode **f, char **rootnames, char **varnames, int *auxids, int mode, Dddmp_VarInfoType varinfo, Dddmp_MoreDDHeaderInfo* extraInfo, char *fname, FILE *fp);
...
extern  DdNode * DddmpextraZddCombination(  DdManager* dd,   int* VarValues,  int nVars );
extern  DdNode * DddmpExtra_zddCombination(   DdManager *dd,   int* VarValues,   int nVars );
extern  DdNode *DddmpExtra_zddSupport(  DdManager * dd,  DdNode * f);
/**AutomaticEnd***************************************************************/
