...
static int DddmpCuddDdArrayLoad(Dddmp_DecompType ddType, DdManager *ddMgr, Dddmp_RootMatchType rootMatchMode, char **rootmatchnames, Dddmp_VarMatchType varMatchMode, char **varmatchnames, int *varmatchauxids, int *varcomposeids, int mode,  char ** extrainfo, char *file, FILE *fp, DdNode ***pproots);
static Dddmp_Hdr_t * DddmpBddReadHeader(char *file, FILE *fp);
static Dddmp_Hdr_t * DddmpZddReadHeader(char *file, FILE *fp);
...
/**Function********************************************************************
   ...
   Synopsis     [Reads a dump file representing the argument ZDD.]

   Description  [Reads a dump file representing the argument ZDD.
...
Dddmp_cuddZddLoad (
		   ...
		   char ** extrainfo               /* OUT: extra info from header */,
		   ...
		   )
{
  ...
  nRoots = Dddmp_cuddZddArrayLoad(ddMgr,DDDMP_ROOT_MATCHLIST,NULL,
                                  varMatchMode,varmatchnames,varmatchauxids,varcomposeids,
                                  mode,extrainfo,file,fp,&tmpArray);
  if (nRoots == 0) {
    return (NULL);
  } else {
    f = tmpArray[0];
    if (nRoots > 1) {
      fprintf (stderr,
	       "Warning: %d ZDD roots found in file. Only first retrieved.\n",
	       nRoots);
      for (i=1; i<nRoots; i++) {
        Cudd_RecursiveDerefZdd (ddMgr, tmpArray[i]);
      } 
    } 
    DDDMP_FREE (tmpArray);
    return (f);
...
/**Function********************************************************************
   ...
   Synopsis    [Reads a dump file representing the argument ZDDs.]

   Description [Reads a dump file representing the argument ZDDs. The header is
			...
			)
{
  ...
  if (retValueBis == 1) {
    fprintf (stderr, "Inconsistency Found During ZDD Load.\n");
    fflush (stderr);
  } else {
    if (retValueBis == CUDD_OUT_OF_MEM) {
      fprintf (stderr, "Out of Memory During ZDD Load.\n");
      fflush (stderr);
    }
  }
#endif
  ...
  //  Dddmp_DecompType ddType = DddmpDdReadType (file, fp);

  //  if(ddType == DDDMP_ZDD)
  //    {
      
      retValue = DddmpCuddDdArrayLoad (DDDMP_ZDD, ddMgr, rootMatchMode,
				       rootmatchnames, varMatchMode, varmatchnames, varmatchauxids,
				       varcomposeids, mode, extrainfo, file, fp, pproots);
      //    }
      //  else
      //    {
      //      fprintf (stderr, "Attempting to load the incorrect DD type.\n");
      //      fflush (stderr);
      //    }
		   ...
		   )
{
  ...
  if (nRoots == 0) {
    ...
    if (nRoots > 1) {
      fprintf (stderr,
	       "Warning: %d ADD roots found in file. Only first retrieved.\n",
	       nRoots);
      for (i=1; i<nRoots; i++) {
        Cudd_RecursiveDeref (ddMgr, tmpArray[i]);
      } 
                      ...
                      )
{
  ...
  int i, j, k, maxaux, var, vT, vE, idT, idE, mvars = 0;
  double addConstant;
  ...
  *pproots = NULL;
  ...
  if ( DDDMP_ZDD == ddType)
    {
      nddvars = ddMgr->sizeZ;
      Hdr = DddmpZddReadHeader (NULL, fp);
    }
  else
    {
      nddvars = ddMgr->size;
  ...
  if (Hdr->ddType != ddType) {
    ...
    if (Hdr->ddType == DDDMP_ZDD)
      (void) fprintf (stderr, "ZDD found\n");
    ...
    if (ddType == DDDMP_ZDD)
      (void) fprintf (stderr, "when loading an ZDD\n");
  ...
  if(Hdr->extraInfo != NULL)
    {
      if(extrainfo == NULL)
	{
	  //! if we have been handed NULL, deal
	  //! with it in a smart way
	  extrainfo = DDDMP_ALLOC(char*,1);
	}
      *extrainfo = DddmpStrDup(Hdr->extraInfo);
    }
 ...
 again_matchmode:
  switch (varMatchMode) {
  case DDDMP_VAR_USEPERMIDS:
    /*
      first set up the variable ordering on the current
      manager as the ordering used in this file
    */
    if (ddType == DDDMP_BDD)
      {
	/* check the size */
	mvars = Cudd_ReadSize(ddMgr);
	//	  printf("New manager size:%d, nBDDsize;%d\n", mvars, Hdr->nsuppvars);
	if(Hdr->nsuppvars != mvars){
	  /* 
	     if there are fewer variables in the manager than in this new bdd,
	     add new variables until the number of variables is equal
	  */
	  while (mvars < Hdr->nsuppvars){
	    Cudd_bddNewVar(ddMgr);
	    mvars = Cudd_ReadSize(ddMgr); /* not very efficient */
	  }				
	}
	  
	int * temp_conv = malloc(sizeof(int) * Hdr->nsuppvars);
	
	for (i = 0; i < Hdr->nsuppvars; i++) {
	  temp_conv[Hdr->permids[i]] = i;
	} 
	
	/* shuffle the variables in the manager to match the order in the BDD */
	Cudd_ShuffleHeap(ddMgr, temp_conv);
	...
	/*
	  int k = 0;
	  for(k = 0; k < 128; k++)
	  printf("position %d has variable %d\n", k, Cudd_ReadPerm(ddMgr,k));
	*/

	
	/* 	
		set up the perm ID matching in the same manner as 
	...
	*/
    ...
    if (ddType == DDDMP_ZDD)
      {
	/* 
	   if there are fewer variables in the manager than in this new zdd,
	   add new variables until the number of variables is equal
	*/
	
	if (Cudd_ReadZddSize(ddMgr) < Hdr->nVars)
	  {
	    Dddmp_CheckAndGotoLabel (0 == (int)(cuddResizeTableZdd(ddMgr, (Hdr->nVars - 1))), 
				     "Error resizing ZDD table.", failure);
	  }
	  
	int * temp_conv = malloc(sizeof(int) * Hdr->nVars);
	
	for (i = 0; i < Hdr->nVars; i++) 
	  {
	    temp_conv[Hdr->varpermids[i]] = i;
	  } 
	
	/* shuffle the variables in the manager to match the order in the ZDD */
	Cudd_zddShuffleHeap(ddMgr, temp_conv);
	free(temp_conv);
	
	/*	
	int k = 0;
	for(k = 0; k < 128; k++)
	  printf("variable %d is in position %d\n", k, Cudd_ReadPermZdd(ddMgr,k));
	*/
	...
	*/
	for (i=0; i<Hdr->nsuppvars; i++) {
	  convertids[permsupport[i]] = Cudd_ReadInvPermZdd (ddMgr,
							    Hdr->permids[i]);
	}
      }
    break;
  ...
  case DDDMP_VAR_MATCHAUXIDS:
    if (Hdr->auxids == NULL) {
      ...
      (void) fprintf (stderr, "but .auxids not found in ZDD file\n");
  ...
  case DDDMP_VAR_MATCHNAMES:
    if (Hdr->suppVarNames == NULL) {
      ...
      (void) fprintf (stderr, "but .suppvarnames not found in ZDD file\n");
      (void) fprintf (stderr, "Matching IDs forced.\n");
      fflush (stderr);
      varMatchMode = DDDMP_VAR_MATCHIDS;
      goto again_matchmode;
  ...
  int dddmp_one_index = -1;
  int dddmp_zero_index = -1;
  ...
  for (i=1; i<=Hdr->nnodes; i++) {
	...
	{
	case DDDMP_TERMINAL:     
	  /* only 1 terminal presently supported for BDDs */    
	  if ((code.Ecompl) && (ddType == DDDMP_ZDD)) 
	    {
	      dddmp_zero_index = i;
	      pnodes[i] = Cudd_ReadZero (ddMgr);
	    }
	...
	{
	case DDDMP_TERMINAL:
	  Dddmp_CheckAndGotoLabel (dddmp_one_index == -1, 
				   "Constant One Index is not set", failure);
	...
	case DDDMP_TERMINAL:  
   	  if ((code.Ecompl) && (ddType == DDDMP_ZDD)) 
	    {
	      Dddmp_CheckAndGotoLabel (dddmp_zero_index == -1, 
				       "Constant Zero Index is not set", failure);	      
	      idE = dddmp_zero_index;
	    }
	  else
	    {
	      Dddmp_CheckAndGotoLabel (dddmp_one_index == -1, 
    ...
    case DDDMP_BDD: 
      ...
      break;
    case DDDMP_ZDD: 
      pnodes[i] = mg_cuddZddGetNodeIVO2(ddMgr, var, T, E);
      break;
    case DDDMP_ADD: 
      { 
  ...
  for (i=1; i<=Hdr->nnodes; i++) {
    ...
    switch (ddType) {
    case DDDMP_ZDD: 
      Cudd_RecursiveDerefZdd (ddMgr, f);
      break;
    case DDDMP_BDD: 
    case DDDMP_ADD: 
    case DDDMP_CNF:
		    ...
		    )
{
  ...
  while (fscanf(fp, "%s", buf)!=EOF) {
    ...
    if (matchkeywd(buf, ".extraddinfo")) {    
      retValue = fscanf (fp, "%s", buf);
      Dddmp_CheckAndGotoLabel (retValue==EOF, "Error reading from file.",
			       failure);
      Hdr->extraInfo = DddmpStrDup(buf);
      continue;
    }
    ...
    if (matchkeywd(buf, ".zdd")) {    
      Hdr->ddType = DDDMP_ZDD;
      continue;
    }
		    ...
		    )
{
  ...
  while (fscanf(fp, "%s", buf)!=EOF) {
    ...
    if (matchkeywd(buf, ".varpermids")) {
      Hdr->varpermids = DddmpIntArrayRead(fp,Hdr->nVars);
      Dddmp_CheckAndGotoLabel (Hdr->varpermids==NULL,
			       "Error allocating memory.", failure);
		 ...
		 )
{
  char buf[DDDMP_MAXSTRLEN];
  int fileToClose = 0;
  Dddmp_DecompType myDdType = DDDMP_NONE;
  ...
  // Begin reading the header looking for
  // the DD type information.
  while ((fscanf(fp, "%s", buf)!=EOF) && 
	 (myDdType == DDDMP_NONE)) 
    {
