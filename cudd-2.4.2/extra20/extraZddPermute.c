...
/**Function********************************************************************
  ...
  the ZDD f.  The size is given by n. The two sets of variables are
  assumed to be disjoint. Returns a pointer to the resulting ZDD if
  successful; NULL otherwise.]

  SideEffects [None]

  SeeAlso     [Extra_ZddPermute Cudd_bddSwapVariables]

******************************************************************************/
DdNode *
mg_Extra_zddSwapVariables(
  DdManager * dd,
  DdNode * f,
  DdNode ** x,
  DdNode ** y,
  int  n)
{
    ...
    permut = ALLOC(int,dd->sizeZ);
    if (permut == NULL) {
	dd->errorCode = CUDD_MEMORY_OUT;
	return(NULL);
    }
    for (i = 0; i < dd->sizeZ; i++) permut[i] = i;
    for (i = 0; i < n; i++) {
	j = x[i]->index;
	k = y[i]->index;
	permut[j] = k;
    ...
    swapped = Extra_zddPermute(dd,f,permut);
    FREE(permut);

    return(swapped);

} /* end of mg_Extra_zddSwapVariables */
...
******************************************************************************/
static int zRefCache = 0;
static int zRefCache1 = 0;
  ...
  DdNode * cube)
{
    ...
    else
    {
	    ...
	    /* if(Cudd_CheckKeys(dd) != 0) */
	    /*   { */
	    /* 	(void) fprintf(dd->err, "ExistAbstract messed up the keys before union zRes0\n"); */
	    /* 	return NULL; */
	    /*   } */
	    ...
	    /* if(Cudd_CheckKeys(dd) != 0) */
	    /*   { */
	    /* 	(void) fprintf(dd->err, "ExistAbstract messed up the keys before getnode zRes0\n"); */
	    /* 	return NULL; */
	    /*   } */
	    ...
	    /* if(Cudd_CheckKeys(dd) != 0) */
	    /*   { */
	    /* 	(void) fprintf(dd->err, "ExistAbstract messed up the keys before getnode zRes1\n"); */
	    /* 	return NULL; */
	    /*   } */
